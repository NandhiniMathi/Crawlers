...
def main():
    try:
        ...
        req = Request('https://seclists.org/rss/bugtraq.rss', headers={'User-Agent': 'Mozilla/5.0'} )
        ...
        for item in items:
            ...
            t=soup.find_all('div',{'class':'contentrecord'})
            soup1=BeautifulSoup(t[0].text,'html.parser')
            for div in soup1.find_all('strong'):
                print(div.text)
            for div2 in soup1.find_all('div',{'class':'content nonfileattachment'}):                
                print(div2.text)
            for div1 in soup1.find_all('p'):                
                print(div1.text)
                
            for div in soup1.find_all('ul',{'class':'footer-list'}):
                div.decompose()
            for div in soup1.find_all('table',{'summary':'components:content:c_root:attributes'}):
                div.decompose()
