...
def main():
    try:
        proxy_support = urllib.request.ProxyHandler({'https' : 'https://672164:'+passw+'@proxy.cognizant.com:6050'})
        auth = urllib.request.HTTPBasicAuthHandler()
        opener = urllib.request.build_opener(proxy_support, auth, urllib.request.HTTPHandler)
        urllib.request.install_opener(opener)
        req = Request('https://blog.avast.com/rss.xml', headers={'User-Agent': 'Mozilla/5.0'} )
        ...
        for item in items:
            ...
            response= urllib.request.urlopen(req)
            soup=BeautifulSoup(response.read(),'html.parser')
            description = soup.find_all('span', {'class':'hs_cos_wrapper hs_cos_wrapper_meta_field hs_cos_wrapper_type_rich_text'})
            ...
            #print('###################\nTitle:',title,'\n\n',desc,'\n\n\n\nContent1::::',content1.get_text(),'\n\nSource:',link,'\n\nCategory:',category,'\n\nCVE:',cve,pubDate,'\n###################')
             '''
            print('\n\nTitle : ',title,'\n\nSource : ',link,'\n\nPublished Date :',pubdate,'\n\nDescription :',content1.get_text(),description[0].get_text(),'\n\nCategory :',category)
    except Exception as e:
        print('Error : ',e)  
